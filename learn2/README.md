...
- [前提条件](#前提条件)
- [1. Multipass のインストール](#1-multipass-のインストール)
- [2. USB SSD の準備](#2-usb-ssd-の準備)
- [3. Multipass VM の作成](#3-multipass-vm-の作成)
- [4. learn2 ディレクトリを VM にマウント](#4-learn2-ディレクトリを-vm-にマウント)
- [5. USB SSD を VM にマウント (NFS 方式)](#5-usb-ssd-を-vm-にマウント-nfs-方式)
- [6. VM 内での SSD 設定](#6-vm-内での-ssd-設定)
- [7. k3s のインストール (コントロールプレーン)](#7-k3s-のインストール-コントロールプレーン)
- [8. k9s のインストール](#8-k9s-のインストール)
- [9. SSD を Kubernetes の PersistentVolume として設定](#9-ssd-を-kubernetes-の-persistentvolume-として設定)
- [10. クラスタの動作確認](#10-クラスタの動作確認)
- [11. ストレージへの書き込み確認](#11-ストレージへの書き込み確認)
- [12. VM の停止・再起動](#12-vm-の停止再起動)
...
## 4. learn2 ディレクトリを VM にマウント

このリポジトリ内の YAML ファイルを VM 内から直接使用するために、learn2 ディレクトリを VM にマウントします。
これにより、`kubectl apply -f` 実行時にファイルを VM へ個別にコピーする手間が省けます。

### 4-1. multipass mount を試みる

```bash
# Mac 側で実行 (learn2 ディレクトリのパスは環境に合わせて変更)
multipass mount /path/to/learn-kubernetes/learn2 k3s-master:/home/ubuntu/learn2
```
...
### 4-2. multipass mount が使えない場合: ファイルを転送する

`multipass transfer` でファイルを個別に VM へコピーできます。

```bash
# learn2 ディレクトリ内の yaml ファイルをまとめて VM へ転送
multipass transfer \
  storageclass-ssd.yaml pv-ssd.yaml pvc-ssd.yaml test-pod.yaml \
...
### 5-3. Mac 側で NFS サーバーを設定

`/etc/exports` に SSD のパスとアクセス許可するサブネットを追加:

```bash
sudo sh -c 'echo "/Volumes/SSD -network 192.168.64.0 -mask 255.255.255.0 -alldirs -mapall=$(id -u):$(id -g)" >> /etc/exports'
```
...
### 5-5. VM 内で NFS クライアントをインストールしてマウント
...
## 9. SSD を Kubernetes の PersistentVolume として設定
...
### 9-2. StorageClass の適用 (`storageclass-ssd.yaml`)

**StorageClass** は、ストレージの種類と動作を定義するリソースです。PV・PVC の `storageClassName` フィールドで参照する名前 (`local-ssd`) をここで定義します。
...
```bash
sudo kubectl apply -f /home/ubuntu/learn2/storageclass-ssd.yaml
```

### 9-3. PersistentVolume マニフェストの適用 (`pv-ssd.yaml`)
...
| `storageClassName` | `local-ssd` | 9-2 で作成した StorageClass 名 |
...
```bash
sudo kubectl apply -f /home/ubuntu/learn2/pv-ssd.yaml
```

### 9-4. PersistentVolumeClaim の適用 (`pvc-ssd.yaml`) (動作確認用)
...
```bash
sudo kubectl apply -f /home/ubuntu/learn2/pvc-ssd.yaml
...
```bash
sudo kubectl apply -f /home/ubuntu/learn2/test-pod.yaml
...
sudo kubectl delete -f /home/ubuntu/learn2/test-pod.yaml
```
